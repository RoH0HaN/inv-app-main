...
<head>
    ...
    <title>Return Amount History | Fast Forward</title>
</head>
<body>
    @extends("main.layout")
    ...
    @section("content")
        <section>
            <x-breadcrumb :links="[
                ...
                ['url' => route('customer.returnAmountHistory', $customer->id), 'text' => 'Return Amount History']
            ]" />

            <!-- Table Start -->
                 <div class="shadow-md rounded-lg bg-[#fff]">
                    <div class="flex justify-between border-b-[1.5px] border-[#dddddd] px-5 py-3">
                        <h3 class="font-semibold text-2xl">RETURN AMOUNT HISTORY</h3>
                        <button type="button" class="inline-flex items-center gap-x-2 text-sm font-medium rounded-full border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-hidden focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                            <a href="{{ route('customer.returnAmountEntry', $customer->id) }}" class="text-[#fff] font-semibold text-sm uppercase py-2 px-5">Return Amount Entry</a>
                        </button>
                    </div>
                    ...
                    <div class="px-5 py-5">
                        ...
                        <div class="flex justify-end items-center space-x-4 mb-4">
                            ...
                            <div class="flex items-center space-x-2">
                                <form method="GET" action="{{ route('customer.returnAmountHistory', $customer->id) }}">
                            ...
                            <table id="myTable" class="custom-data-table">
                                ...
                                <tbody>
                                    @foreach($returnAmountTransactions as $returnAmountTransaction)
                                    <tr>
                                        <td>{{ $returnAmountTransaction->date }}</td>
                                        <td>{{ $returnAmountTransaction->referral_number }}</td>
                                        <td>₹ {{ $returnAmountTransaction->amount }}</td>
                                        <td>{{ $customer->first_name }} {{ $customer->last_name }}</td>
                                        <td>{{ $returnAmountTransaction->particular }}</td>
                                        <td>{{ $returnAmountTransaction->created_by_first_name }} {{ $returnAmountTransaction->created_by_last_name }}</td>
                                        <td>
                            ...
                            {{ $returnAmountTransactions->appends(['search' => request('search')])->links() }}
                        </div>
                    </div>
                 </div>
                <!-- Table End -->
