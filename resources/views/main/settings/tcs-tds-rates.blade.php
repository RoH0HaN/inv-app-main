...
<head>
    ...
    <title>Tds/Tcs Rates | Fast Forward</title>
</head>
<body>
    @extends("main.layout")
         ...
         <!-- For success message -->
            ...
            <div class="shadow-md rounded-lg bg-[#fff]">
                <div class="flex justify-between border-b-[1.5px] border-[#dddddd] px-5 py-3">
                    <h3 class="font-semibold text-2xl">TDS/TCS RATES</h3>
                    <button type="button" class="inline-flex items-center gap-x-2 text-sm font-medium rounded-full border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-hidden focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                        <a href="/settings/create-tcs-tds" class="text-[#fff] font-semibold text-sm uppercase py-2 px-5">Create TDS/TCS</a>
                    </button>
                </div>
                <div class="px-5 py-5">
                    <div class="table-container">
                        <div class="flex justify-end items-center space-x-4 mb-4">
                            ...
                            <div class="flex items-center space-x-2">
                                <form method="GET" action="{{ route('settings.tcsTdsRates') }}">
                        ...
                        <table id="myTable">
                            ...
                            <tbody>
                                @foreach($tcsTdsRates as $tcsTdsRate)
                                <tr>
                                    <td>{{ $tcsTdsRate->name }}</td>
                                    <td>{{ $tcsTdsRate->rate }}</td>
                                    <td>{{ $tcsTdsRate->type }}</td>
                                    <td>{{ $tcsTdsRate->first_name }} {{ $tcsTdsRate->last_name }}</td>
                                    <td>{{ $tcsTdsRate->created_at }}</td>
                                    <td>
                                        <div class="hs-dropdown relative inline-flex">
                                            ...
                                            <div class="hs-dropdown-menu transition-[opacity,margin] z-20 duration hs-dropdown-open:opacity-100 opacity-0 hidden min-w-60 bg-white rounded-lg mt-2 divide-y divide-gray-200 dark:bg-neutral-800 dark:border dark:border-neutral-700 dark:divide-neutral-700 shadow-2xl" role="menu" aria-orientation="vertical" aria-labelledby="hs-dropdown-with-icons">
                                                <div class="p-1">
                                                    <button class="flex items-center w-full cursor-pointer gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300 dark:focus:bg-neutral-700" aria-haspopup="dialog" aria-expanded="false" aria-controls="edit-tcs-tds-dialog" data-hs-overlay="#edit-tcs-tds-dialog"
                                                    data-id="{{ $tcsTdsRate->id }}"
                                                    data-name="{{ $tcsTdsRate->name }}"
                                                    data-rate="{{ $tcsTdsRate->rate }}"
                                                    data-type="{{ $tcsTdsRate->type }}"
                                                    >
                                                    ...
                                                    aria-haspopup="dialog" aria-expanded="false" 
                                                        ...
                                                        data-name="{{ $tcsTdsRate->name }}"
                                                        data-id="{{ $tcsTdsRate->id }}"
                                                        data-action="{{ route('settings.deleteTcsTdsToDatabase') }}"
                                                    >
                        ...
                        {{ $tcsTdsRates->appends(['search' => request('search')])->links() }}
                    </div>
                </div>
            </div>
            <!-- Table End -->

            <x-edit-dialogs.tcs-tds-edit-component />
            <x-delete-dialog />
        </section>
    @endsection
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const editButtons = document.querySelectorAll('[data-hs-overlay="#edit-tcs-tds-dialog"]');
            ...
            button.addEventListener('click', () => {
                ...
                // Select the right radio button manually
